...
class AutotradeWorker():
    ...
    def execute(self):
        ...
        trade_id, action, price, position, profit = self.drivers['api'].collect_data(action_json)
        self.drivers['store'].put_trade_history(trade_id, action_json.get("action"), price, position, profit, datetime.datetime.now())
